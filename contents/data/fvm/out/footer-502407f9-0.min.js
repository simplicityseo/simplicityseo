...
MYMAP.init=function(selector,latLng,zoom){if(typeof wpgmaps_localize[wpgmaps_mapid].type!=="undefined"){if(wpgmaps_localize[wpgmaps_mapid].type==="1"){maptype=google.maps.MapTypeId.ROADMAP}else if(wpgmaps_localize[wpgmaps_mapid].type==="2"){maptype=google.maps.MapTypeId.SATELLITE}else if(wpgmaps_localize[wpgmaps_mapid].type==="3"){maptype=google.maps.MapTypeId.HYBRID}else if(wpgmaps_localize[wpgmaps_mapid].type==="4"){maptype=google.maps.MapTypeId.TERRAIN}else{maptype=google.maps.MapTypeId.ROADMAP}}else{maptype=google.maps.MapTypeId.ROADMAP}
...
if(!wpgmaps_localize[wpgmaps_mapid].other_settings.wpgmza_show_points_of_interest){if(!myOptions.styles)
...
var point=new google.maps.LatLng(parseFloat(lat),parseFloat(lng));MYMAP.bounds.extend(point);if(show_marker_radius===!0){if(wpmgza_anim==="1"){var marker=new google.maps.Marker({position:point,map:MYMAP.map,animation:google.maps.Animation.BOUNCE})}else if(wpmgza_anim==="2"){var marker=new google.maps.Marker({position:point,map:MYMAP.map,animation:google.maps.Animation.DROP})}else{var marker=new google.maps.Marker({position:point,map:MYMAP.map})}
...
var html='<span style=\'min-width:100px; display:block;\'>'+wpmgza_address+'</span>'+d_string;if(wpmgza_infoopen==="1"&&!wpgmaps_localize_global_settings.wpgmza_settings_disable_infowindows){infoWindow.setContent(html);infoWindow.open(MYMAP.map,marker)}
temp_actiontype='click';if(typeof wpgmaps_localize_global_settings.wpgmza_settings_map_open_marker_by!=="undefined"&&wpgmaps_localize_global_settings.wpgmza_settings_map_open_marker_by=='2'){temp_actiontype='mouseover'}
google.maps.event.addListener(marker,temp_actiontype,function(){infoWindow.close();if(!wpgmaps_localize_global_settings.wpgmza_settings_disable_infowindows){infoWindow.setContent(html);infoWindow.open(MYMAP.map,marker)}})}}});if(''!==jQuery('#addressInput').val()&&markerStoreLocatorsNum<1){slNotFoundMessage.addClass('is-active');setTimeout(function(){slNotFoundMessage.removeClass('is-active')},5000)}}}}}
...
if(!wpgmaps_localize_global_settings.wpgmza_settings_disable_infowindows){infoWindow_poly[polygonid].setContent(content);infoWindow_poly[polygonid].open(MYMAP.map,this.position)}})}}
